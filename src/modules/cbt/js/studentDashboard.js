...
const studentDashboard = {
    ...
    init: async () => {
        // If module guard blocked this page, skip all CBT runtime
        if (window.__cbtBlocked) return;
    ...
    populateSubjectFilters: () => {
        ...
        const opts = subjects.map(s => {
            const escaped = studentDashboard._escapeHtml(s);
            return `<option value="${escaped}">${s === 'All' ? 'All Subjects' : escaped}</option>`;
        }).join('');
    ...
    renderAvailable: () => {
            ...
            html += actionItems.map(result => {
                ...
                <div class="exam-list-item action-required" onclick="studentDashboard.reviewExam('${result.examId}', '${result.id}')">
                    ...
                    <div class="exam-list-info">
                        <div class="exam-list-title" style="font-size:1.05rem; font-weight:800; color:var(--text-color);">${studentDashboard._escapeHtml(exam.subject)}</div>
                        <div class="exam-list-subtitle">${studentDashboard._escapeHtml(exam.title)} · Action Required · ${timeLeft}m left</div>
                    </div>
        ...
        const available = studentDashboard.exams.filter(e => {
            ...
            if (e.extensions && e.extensions._adminLocked) {
                return false;
            }
            ...
            <div class="exam-list-item ${isScheduled && !isInProgress ? 'scheduled' : ''}" data-exam-id="${exam.id}" onclick="studentDashboard.selectExam('${exam.id}', 'available')">
                ...
                <div class="exam-list-info" style="flex:1; min-width:0;">
                    ...
                    <div style="display:flex; justify-content:space-between; align-items:center;">
                        <div class="exam-list-subtitle" style="font-size:0.78rem; color:var(--light-text); font-weight:400;">${studentDashboard._escapeHtml(exam.title)}${isInProgress ? ' · In Progress' : ''}</div>
    ...
    renderResolved: () => {
        ...
        grid.innerHTML = resolvedItems.map(result => {
            ...
            <div class="exam-list-item resolved" data-result-id="${result.id}" onclick="window.location.href='results.html?id=${result.id}'">
                ...
                <div class="exam-list-info">
                    <div class="exam-list-title" style="font-size:1.05rem; font-weight:800; color:var(--text-color);">${studentDashboard._escapeHtml(exam.subject)}</div>
                    <div class="exam-list-subtitle">${studentDashboard._escapeHtml(exam.title)} · Resolved · ${points}/${totalPoints} pts</div>
                </div>
                <div class="exam-list-status">
    ...
    renderCompleted: () => {
        ...
        grid.innerHTML = completedResults.map(result => {
            ...
            <div class="exam-list-item completed" data-result-id="${result.id}" onclick="window.location.href='results.html?id=${result.id}'">
                ...
                <div class="exam-list-info">
                    <div class="exam-list-title" style="font-size:1.05rem; font-weight:800; color:var(--text-color);">${studentDashboard._escapeHtml(exam.subject)}</div>
                    <div class="exam-list-subtitle">${studentDashboard._escapeHtml(exam.title)} · ${points}/${totalPoints} pts${theoryLabel}</div>
                </div>
                <div class="exam-list-status">
        ...
        if (centerContent) {
                ...
                customInstructionsHtml = `
                    <div style="margin-bottom: 20px;">
                        ...
                        <div style="background: var(--card-bg, #fff); padding: 12px; border-left: 3px solid var(--primary-color); border-radius: 4px; color: var(--text-color);">
                            ${studentDashboard._escapeHtml(exam.instructions)}
                        </div>
                    </div>
                `;
            }
            ...
            centerContent.innerHTML = `
                <div class="exam-detail-header">
                    <h2 class="exam-detail-title" style="font-size:1.5rem; font-weight:800;">${studentDashboard._escapeHtml(exam.subject)}</h2>
                    <span class="exam-detail-breadcrumb">${studentDashboard._escapeHtml(exam.title)} · ${studentDashboard._escapeHtml(exam.targetClass || 'All Classes')}</span>
                </div>
        ...
        if (metaContent) {
            ...
            metaContent.innerHTML = `
                <div class="meta-row">
                    ...
                    <div class="meta-value">${studentDashboard._escapeHtml(exam.title || 'N/A')}</div>
                </div>
                <div class="meta-row">
                    ...
                    <div class="meta-value">${studentDashboard._escapeHtml(exam.subject)}</div>
                </div>
                <div class="meta-row">
    ...
    startExam: async (examId) => {
        ...
        if (!availability.available) {
            ...
            if (availability.reason === 'admin_locked') {
                await Utils.showAlert('Exam Locked', 'This exam has been temporarily locked by the school administrator.');
                return;
            }
        }

        const confirmed = await Utils.showConfirm('Start Exam', `Are you sure you want to start <strong>${studentDashboard._escapeHtml(exam.subject)} — ${studentDashboard._escapeHtml(exam.title)}</strong>?<br><br>The timer will begin immediately once you proceed.`);
        if (confirmed) {
            ...
            try {
                ...
                if (navigator.onLine) {
                    try {
                        ...
                        const fieldsParam = 'fields=' + encodeURIComponent('updated,scheduled_date,status,extensions');
