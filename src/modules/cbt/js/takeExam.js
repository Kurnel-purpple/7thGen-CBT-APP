...
const takeExam = {
    ...
    openSplitImagePanel: (index = takeExam.currentQuestionIndex, preferredSrc = null) => {
        ...
        inner.className = 'exam-image-panel-inner';
        const header = document.createElement('div');
        header.className = 'exam-image-panel-header';
        const headerRow = document.createElement('div');
        headerRow.className = 'exam-image-panel-header-row';
        const leftDiv = document.createElement('div');
        const kicker = document.createElement('span');
        kicker.className = 'exam-image-panel-kicker';
        kicker.textContent = `Question ${questionNumber}`;
        leftDiv.appendChild(kicker);
        const h3 = document.createElement('h3');
        h3.textContent = `Reference Image${assets.length > 1 ? 's' : ''}`;
        leftDiv.appendChild(h3);
        headerRow.appendChild(leftDiv);
        const closeBtn = document.createElement('button');
        closeBtn.type = 'button';
        closeBtn.className = 'exam-image-panel-close';
        closeBtn.setAttribute('aria-label', 'Close split image view');
        closeBtn.textContent = '×';
        closeBtn.addEventListener('click', takeExam.closeSplitImagePanel);
        headerRow.appendChild(closeBtn);
        header.appendChild(headerRow);
        inner.appendChild(header);
        const stack = document.createElement('div');
        stack.className = 'exam-image-stack';
        assets.forEach((asset, assetIndex) => {
            const frame = document.createElement('div');
            frame.className = 'exam-image-frame';
            const img = document.createElement('img');
            img.src = typeof asset.src === 'string' ? asset.src : '';
            img.alt = typeof asset.label === 'string' ? asset.label : `Question ${questionNumber} image ${assetIndex + 1}`;
            frame.appendChild(img);
            const footer = document.createElement('div');
            footer.className = 'exam-image-frame-footer';
            const span = document.createElement('span');
            span.textContent = typeof asset.label === 'string' ? asset.label : `Image ${assetIndex + 1}`;
            footer.appendChild(span);
            const btn = document.createElement('button');
            btn.type = 'button';
            btn.className = 'exam-image-panel-btn';
            btn.textContent = 'Open Full Screen';
            btn.addEventListener('click', () => takeExam.showImageLightbox(typeof asset.src === 'string' ? asset.src : ''));
            ...
            if (q.topInstruction) {
                sectionHeader += `
                    <div style="margin: 14px 0 4px; padding: 10px 16px;
                        ...
                        ${takeExam._escapeHtml(q.topInstruction)}
                    </div>
                `;
            }

            if (q.type === 'theory' && index === sortedQuestions.findIndex(qq => qq.type === 'theory')) {
                ...
                sectionHeader += `
                    <div style="margin: 40px 0 20px 0; padding: 15px; background: var(--inner-bg); border-left: 4px solid var(--accent-color); border-radius: 4px;">
                        ...
                        <p style="margin: 5px 0 0 0; font-size: 0.9rem; color: var(--light-text);">${takeExam._escapeHtml(theoryInstructions)}</p>
                    </div>
                `;
            }
            ...
            if (!q.type || q.type === 'mcq' || q.type === 'image_mcq') {
                ...
                optionsHtml = imgHtml + q.options.map((opt) => {
                    ...
                    <label class="option-label ${isSelected ? 'selected' : ''}">
                        ...
                        <span>${takeExam._escapeHtml(opt.text)}</span>
                    </label>
                    `;
                }).join('');
                ...
                const val = takeExam._escapeHtml(takeExam.answers[q.id] || '');
                optionsHtml = `
                ...
                optionsHtml = q.pairs.map((pair, idx) => {
                    ...
                    const dropdownOpts = q.pairs.map(p => `
                        <option value="${takeExam._escapeHtml(p.right)}" ${currentVal === p.right ? 'selected' : ''}>${takeExam._escapeHtml(p.right)}</option>
                    `).join('');

                    return `
                    <div style="display:flex; align-items:center; margin-bottom:10px; gap:10px;">
                        <span style="flex:1; font-weight:500;">${takeExam._escapeHtml(pair.left)}</span>
                        <span>=</span>
