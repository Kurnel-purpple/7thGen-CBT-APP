...
const examManager = {
    ...
    loadExam: async (id) => {
        try {
            ...
            if (scheduledDateInput && exam.scheduledDate) {
                ...
                scheduledDateInput.value = `${date.getFullYear()}-${pad(date.getMonth() + 1)}-${pad(date.getDate())}T${pad(date.getHours())}:${pad(date.getMinutes())}`;
            }
    ...
    applyDraft: (draft) => {
        ...
        examManager.currentExamId = null;
        examManager.questions = Array.isArray(draft.questions) ? draft.questions.map(q => ({ ...q })) : [];
        examManager.uploadedMedia = [];
        ...
        const passScoreInput = document.getElementById('exam-pass-score');
        const instructionsInput = document.getElementById('exam-instructions');
        const theoryInstructionsInput = document.getElementById('exam-theory-instructions');
        const scheduledDateInput = document.getElementById('exam-scheduled-date');
        const scrambleYes = document.getElementById('exam-scramble-yes');
        const scrambleNo = document.getElementById('exam-scramble-no');

        if (schoolLevelSelect && draft.schoolLevel) {
            schoolLevelSelect.value = draft.schoolLevel;
            schoolLevelSelect.dispatchEvent(new Event('change'));
        }

        // Use a longer delay to allow cascading dropdowns to fully render
        // before setting subject/class values
        setTimeout(() => {
            // Populate the hidden select options from academic data so the value can be set
            if (draft.schoolLevel && typeof window.__academicData !== 'undefined') {
                const data = window.__academicData;
                if (subjectSelect && data.subjectsByLevel?.[draft.schoolLevel]) {
                    // Only populate if empty (not already done by change event)
                    if (subjectSelect.options.length <= 1) {
                        subjectSelect.innerHTML = '<option value="">Select Subject</option>';
                        data.subjectsByLevel[draft.schoolLevel].forEach(s => {
                ...
                if (targetClassSelect && data.classesByLevel?.[draft.schoolLevel]) {
                    if (targetClassSelect.options.length <= 1) {
                        targetClassSelect.innerHTML = '';
                        data.classesByLevel[draft.schoolLevel].forEach(c => {
                            const opt = document.createElement('option');
            ...
            if (subjectSelect && draft.subject) subjectSelect.value = draft.subject;
            if (targetClassSelect && draft.targetClass) targetClassSelect.value = draft.targetClass;
            if (typeof populateChainDropdowns === 'function' && draft.schoolLevel) populateChainDropdowns(draft.schoolLevel);
            if (typeof updateStepsChainState === 'function') updateStepsChainState();
        }, 150);

        if (durationInput && draft.duration) durationInput.value = draft.duration;
        if (passScoreInput && draft.passScore) passScoreInput.value = draft.passScore;
        if (instructionsInput) instructionsInput.value = draft.instructions || '';
        if (theoryInstructionsInput) theoryInstructionsInput.value = draft.theoryInstructions || '';
        if (scheduledDateInput && draft.scheduledDate) scheduledDateInput.value = draft.scheduledDate;
    ...
    saveExam: async (e) => {
        ...
        try {
            if (examManager.currentExamId) {
                ...
                const created = await dataService.createExam(examData);
                if (created && created.id) {
                    examData.id = created.id;
                }
            }
            // Auto-save questions to Question Bank if opted in
            ...
            if (saveToQbRadio && saveToQbRadio.checked && window.__moduleLoader?.isModuleEnabled('question_bank')) {
                try {
                    ...
                    // Don't block the exam save flow if QB import fails
                    console.warn('[QB] Auto-import to question bank failed:', qbErr);
                }
            }
